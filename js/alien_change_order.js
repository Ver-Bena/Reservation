...
function init() {
    document.getElementById("member_rsvForm").onsubmit = Update;
...
function Update() {
    ...
    if (curPeople_Cnt == people_cnt && isCalcul) {
        //당일로부터 최소 7일 이상인가?
        ...
        if (((d_rsv_date.getTime()-today.getTime())/1000/60/60/24) > 6) { // 당일로부터 7일 뒤인가?
            ...
            $.ajax({
                url: "/alien_rsv/change",
                type: "POST",
                data: parse_rsv,
                contentType: "application/x-www-form-urlencoded",
                success: function(result) {
                    alert("수정이 완료되었습니다! 수정된 예약내역이 이메일로 전송되었습니다:)");
