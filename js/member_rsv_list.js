            ...
            if (result) {
                ...
                if (result.result != null) {
                    for (var i = 0; i < result.result.length; i++) {
                        ...
                        var changeBtn = $('<button></button>').addClass('btn btn-success').attr("id", "change"+i).text("변경").click(ChangeOrder);
                        var deleteBtn = $('<button></button>').addClass('btn btn-danger').attr("id", "delete"+i).text("삭제").click(DeleteOrder);

                        row10.append(changeBtn);
...
function DeleteOrder(e) {
    ...
    var del_name = document.getElementById("name"+index).innerHTML;
    var del_tel = document.getElementById("tel"+index).innerHTML;
    var del_grade = document.getElementById("grade"+index).innerHTML;
    var del_rsv_date = document.getElementById("rsv_date"+index).innerHTML;
    var del_rsv_time = document.getElementById("rsv_time"+index).innerHTML;
    var del_people_cnt = document.getElementById("people_cnt"+index).innerHTML;
    var del_requests = document.getElementById("requests"+index).innerHTML;
    ...
    var json_delete = `{ 
        ...
        "tel": "`+del_tel+`",
        "grade": "`+del_grade+`",
        "rsv_date": "`+del_rsv_date+`",
        "rsv_time": "`+del_rsv_time+`",
        "people_cnt": "`+del_people_cnt+`",
